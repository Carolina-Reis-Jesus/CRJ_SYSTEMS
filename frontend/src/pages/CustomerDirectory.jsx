            ...
            <tbody>
              {rows.map((c) => (
                <tr key={c.id} data-testid={`customer-row-${c.id}`} className="border-b border-slate-100 hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-slate-700">{maskEmail(c.email)}</td>
                  <td className="px-4 py-3 font-mono text-xs text-slate-700">{maskPhone(c.phone)}</td>
                  <td className="px-4 py-3 font-mono text-xs text-slate-700">{maskCPF(c.cpf)}</td>
                  <td className="px-4 py-3">
                    <Badge variant="outline" className={cn("text-[10px]", planStyles[c.plan])}>{c.plan}</Badge>
                  </td>
                  <td className="px-4 py-3 text-right text-sm text-slate-900 tabular-nums">R$ {c.mrr.toLocaleString("pt-BR")}</td>
