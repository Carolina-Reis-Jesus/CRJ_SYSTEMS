import { Search, Bell, LogOut, Command, Menu } from "lucide-react";
import { useAuth } from "@/context/AuthContext";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuLabel, DropdownMenuSeparator, DropdownMenuTrigger } from "@/components/ui/dropdown-menu";
import { Badge } from "@/components/ui/badge";

const Topbar = ({ title, subtitle, onOpenMobile }) => {
  const { user, logout } = useAuth();

  return (
    <header
      data-testid="app-topbar"
      className="h-16 sticky top-0 z-30 bg-white/85 backdrop-blur border-b border-slate-200/70 flex items-center justify-between px-4 sm:px-6 gap-3"
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        <button
          ...
          className="lg:hidden h-9 w-9 grid place-items-center rounded-md ring-1 ring-slate-200/80 text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-colors shrink-0"
        >
          <Menu className="h-4 w-4" />
        </button>
        <div className="min-w-0">
          <h1 data-testid="page-title" className="text-sm sm:text-base font-semibold text-slate-900 tracking-tight truncate">
            {title}
          </h1>
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        <div className="hidden md:flex items-center gap-2 h-9 px-3 rounded-md bg-slate-50 ring-1 ring-slate-200/80 text-sm text-slate-500 w-56 xl:w-72">
          <Search className="h-4 w-4" />
          <span className="text-slate-400 truncate">Search tickets, customers…</span>
